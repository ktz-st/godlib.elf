...
void	GraphicCanvas_Init( sGraphicCanvas * apCanvas,const U16 aColourMode,const U16 aWidth,const U16 aHeight )
{
	...
	apCanvas->mpLineOffsets = (U32*)mMEMALLOC( ((U32)apCanvas->mHeight + 1L) * sizeof(U32) );

	for( i=0; i<=apCanvas->mHeight; i++ )
	{
		apCanvas->mpLineOffsets[ i ] = lOffset;
		lOffset += lLineWidth;
	}
...
/*-----------------------------------------------------------------------------------*
* FUNCTION : GraphicCanvas_DeInit( sGraphicCanvas * apCanvas )
* ACTION   : GraphicCanvas_DeInit
* CREATION : 21.05.2026
*-----------------------------------------------------------------------------------*/

void	GraphicCanvas_DeInit( sGraphicCanvas * apCanvas )
{
	if( apCanvas->mpLineOffsets )
	{
		mMEMFREE( apCanvas->mpLineOffsets );
		apCanvas->mpLineOffsets = 0;
	}
}
...
void	Graphic_ChunkList_Store( sGraphicChunkList * apList, sGraphicCanvas * apCanvas, const sGraphicRect * apRect )
{
	...
	U16 lOff = (U16)apCanvas->mpLineOffsets[ lY ];
...
void	Graphic_ChunkList_ReStore( sGraphicChunkList * apList, sGraphicCanvas * apSrc, sGraphicCanvas * apDst )
{
	...
	lAddSrc = (U16)(apSrc->mpLineOffsets[ 1 ]>>1);
	lAddDst = (U16)(apSrc->mpLineOffsets[ 1 ]>>1);
